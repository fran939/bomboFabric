...
public class EggFinder {
        ...
        if (matcher.find()) {
            try {
                ...
                if (BomboConfig.get().eggFinderChat) {
                    client.player.sendSystemMessage(Component.literal("§8[§bBomboAddons§8] §aYou found a ")
                            .append(Component.literal(eggType.name + " Egg").withStyle(style -> style.withColor(eggType.chatColor)))
                            .append(" §aat " + eggPos.getX() + ", " + eggPos.getY() + ", " + eggPos.getZ() + "!"));
                }
    ...
    public static void onWebsocketMessage(String eggTypeStr, BlockPos pos) {
        ...
        if (BomboConfig.get().eggFinderChat && !eggType.collected) {
            ...
            if (client.player != null) {
                ...
                client.player.sendSystemMessage(Component.literal("§8[§bBomboAddons§8] §aNew ")
                                .append(eggName)
    ...
    public static void render(LevelRenderContext context) {
        ...
        for (EggWaypoint wp : wps) {
            ...
            double y = wp.pos.getY() + 0.5 - camPos.y;
            ...
            if (BomboConfig.get().eggFinderBeacon) {
                ...
                collector.submitCustomGeometry(poseStack, RenderTypes.linesTranslucent(), (pose, vertexConsumer) -> {
                    BomboRenderUtils.drawBox(pose.pose(), vertexConsumer, beaconBox, r, g, b, 0.4f, 2.0f);
                });
            }
    ...
    public static String getSkyblockerLocationName(String bomboLocation) {
        if (bomboLocation == null) return null;
        ...
        if (lower.contains("gold mine") || lower.equals("gold") || (lower.contains("gold") && lower.contains("mine"))) {
            return "Gold Mine";
        }
