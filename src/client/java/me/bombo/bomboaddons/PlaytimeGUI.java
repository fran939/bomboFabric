...
public class PlaytimeGUI extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        int clockY = startY;
        graphics.item(Items.CLOCK.getDefaultInstance(), clockX, clockY);
    ...
    private void drawCustomTooltip(GuiGraphicsExtractor graphics, HoveredTooltip tooltip, int mouseX, int mouseY) {
        ...
        graphics.text(this.font, tooltip.title, tX, tY, 0xFFFFFFFF, true);
        for (int i = 0; i < tooltip.lore.length; i++) {
            graphics.text(this.font, tooltip.lore[i], tX, tY + (i + 1) * 10, 0xFFFFFFFF, true);
        }
    }
