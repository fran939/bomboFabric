...
public class ItemListOverlay {
   ...
   private static void filterItems() {
      ...
      if (allItems != null) {
         ...
         String[] tokens = lowerQuery.isEmpty() ? new String[0] : lowerQuery.split(",");
         ...
         for(SkyblockItemManager.SkyblockItemInfo info : allItems.values()) {
            boolean matchesQuery = tokens.length == 0;
            if (!matchesQuery) {
               String infoName = info.name != null ? info.name.toLowerCase(java.util.Locale.ROOT) : "";
               String infoId = info.id != null ? info.id.toLowerCase(java.util.Locale.ROOT) : "";
               for (String t : tokens) {
                  if (!t.isEmpty() && (infoName.contains(t) || infoId.contains(t))) {
                     matchesQuery = true;
                     break;
                  }
               }
               if (!matchesQuery && info.lore != null) {
                  for (Component lineComp : info.lore) {
                     if (lineComp != null) {
                        String lineStr = lineComp.getString().toLowerCase(java.util.Locale.ROOT);
                        for (String t : tokens) {
   ...
   public static void render(GuiGraphicsExtractor graphics, Font font, int mouseX, int mouseY) {
      ...
      if (!BomboConfig.get().itemListEnabled) {
         ...
         boolean autoHide = BomboConfig.get().autoHideItemList && !(Minecraft.getInstance().gui.screen() instanceof HudMoveScreen);
         ...
         if (isHiddenState) {
            ...
            if (Minecraft.getInstance().gui.screen() instanceof HudMoveScreen) {
   ...
   public static boolean mouseClicked(double mouseX, double mouseY, int button) {
      if (!BomboConfig.get().itemListEnabled) {
            ...
            if (searchBox.isMouseOver(unscaledX, unscaledY)) {
               if (button == 1) {
                  ...
                  if (Minecraft.getInstance().gui.screen() != null) {
                     Minecraft.getInstance().gui.screen().setFocused((GuiEventListener)null);
                  }
         ...
         if (isHiddenState) {
            return false;
         } else if (Minecraft.getInstance().gui.screen() instanceof HudMoveScreen) {
            return false;
         } else {
            boolean onLeft = sidebarX < Minecraft.getInstance().getWindow().getGuiScaledWidth() / 2;
            int togglesWidth = 128;
               ...
               if (variants != null) {
                  ...
                  for(int v = 0; v < variants.size(); ++v) {
                     ...
                     if (mouseX >= (double)vx && mouseX < (double)(vx + 16) && mouseY >= (double)popoutY && mouseY < (double)(popoutY + 16)) {
                        ...
                        if (button == 0) {
                           Minecraft.getInstance().setScreenAndShow(new RecipeViewerScreen(info.id, Minecraft.getInstance().gui.screen()));
                        } else if (button == 1) {
                           RecipeViewerScreen rvs = new RecipeViewerScreen(info.id, Minecraft.getInstance().gui.screen());
