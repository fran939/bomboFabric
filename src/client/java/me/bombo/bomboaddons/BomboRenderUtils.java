...
public class BomboRenderUtils {
   ...
   public static void drawFilledBox(PoseStack poseStack, VertexConsumer buffer, AABB aabb, float r, float g, float b, float a) {
      drawFilledBox(poseStack.last().pose(), buffer, (float)aabb.minX, (float)aabb.minY, (float)aabb.minZ, (float)aabb.maxX, (float)aabb.maxY, (float)aabb.maxZ, r, g, b, a);
   }
   ...
   public static int parseHexColorWithAlpha(String colorStr, int alpha, int defaultArgb) {
      if (colorStr == null || colorStr.trim().isEmpty()) {
         return (Math.max(0, Math.min(255, alpha)) << 24) | (defaultArgb & 0x00FFFFFF);
      }
      try {
   ...
   public static void drawBeaconBeam(PoseStack poseStack, OrderedSubmitNodeCollector collector, double x, double y, double z, int colorArgb, long gameTime) {
      int yOffset = 0;
      int height = 256;
      float time = (float) Math.floorMod(gameTime, 40) + Minecraft.getInstance().getDeltaTracker().getGameTimeDeltaPartialTick(true);
      float v1 = -time * 0.2F - (float) Math.floor(-time * 0.2F);
      float v2 = (float) height * (0.5F / 0.2F) + v1;

      poseStack.pushPose();
      poseStack.translate(x + 0.5, y, z + 0.5);

      // Inner beam
      collector.submitCustomGeometry(poseStack, net.minecraft.client.renderer.rendertype.RenderTypes.beaconBeam(net.minecraft.client.renderer.blockentity.BeaconRenderer.BEAM_LOCATION, false), (pose, vc) -> {
         renderBeamSection(pose, vc, colorArgb, yOffset, height, 0.0F, 0.2F, 0.2F, 0.0F, -0.2F, 0.0F, 0.0F, -0.2F, 0.0F, 1.0F, v1, v2);
      });

      // Outer glow
      int glowColor = (colorArgb & 0x00FFFFFF) | 0x20000000;
      collector.submitCustomGeometry(poseStack, net.minecraft.client.renderer.rendertype.RenderTypes.beaconBeam(net.minecraft.client.renderer.blockentity.BeaconRenderer.BEAM_LOCATION, true), (pose, vc) -> {
         renderBeamSection(pose, vc, glowColor, yOffset, height, -0.25F, -0.25F, 0.25F, -0.25F, -0.25F, 0.25F, 0.25F, 0.25F, 0.0F, 1.0F, v1, v2);
      });

      poseStack.popPose();
   }

   private static void renderBeamSection(PoseStack.Pose pose, VertexConsumer vc, int color, int yOffset, int height, float x1, float z1, float x2, float z2, float x3, float z3, float x4, float z4, float u1, float u2, float v1, float v2) {
      int yMin = yOffset;
      int yMax = yOffset + height;
      renderQuad(pose, vc, color, yMin, yMax, x1, z1, x2, z2, u1, u2, v1, v2);
      renderQuad(pose, vc, color, yMin, yMax, x4, z4, x3, z3, u1, u2, v1, v2);
      renderQuad(pose, vc, color, yMin, yMax, x2, z2, x4, z4, u1, u2, v1, v2);
      renderQuad(pose, vc, color, yMin, yMax, x3, z3, x1, z1, u1, u2, v1, v2);
   }

   private static void renderQuad(PoseStack.Pose pose, VertexConsumer vc, int color, int yMin, int yMax, float x1, float z1, float x2, float z2, float u1, float u2, float v1, float v2) {
      addVertex(pose, vc, color, yMax, x1, z1, u2, v1);
      addVertex(pose, vc, color, yMin, x1, z1, u2, v2);
      addVertex(pose, vc, color, yMin, x2, z2, u1, v2);
      addVertex(pose, vc, color, yMax, x2, z2, u1, v1);
   }

   private static void addVertex(PoseStack.Pose pose, VertexConsumer vc, int color, int y, float x, float z, float u, float v) {
