...
public class BomboApiUrl {
   ...
   public static java.net.http.HttpRequest.Builder attachApiKey(java.net.http.HttpRequest.Builder builder) {
      String key = me.bombo.bomboaddons.features.auth.BomboApiKeyManager.getApiKey();
      ...
      try {
         ...
         if (s != null && (s.apiDebug || s.apiKeyDebug)) {
            debug = true;
         }
      } catch (Throwable ignored) {}
      if (debug) {
         String keyDisplay = (key == null || key.isEmpty()) ? "NONE" : key;
         System.out.println("[BomboAPI-KeyDebug] Using API Key: " + keyDisplay);
         try {
            net.minecraft.client.Minecraft mc = net.minecraft.client.Minecraft.getInstance();
            if (mc != null && mc.player != null) {
               mc.execute(() -> mc.player.sendSystemMessage(net.minecraft.network.chat.Component.literal("§8[§bBomboAPI-KeyDebug§8] §7Using key: §b" + keyDisplay)));
            }
         } catch (Throwable ignored) {}
      }
      return builder;
