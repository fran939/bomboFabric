...
public class SkyblockCalculator {
   ...
   public static String formatCompactNumber(double val) {
      ...
      if (abs >= 1_000_000_000_000.0) {
         double t = abs / 1_000_000_000_000.0;
         return sign + (t == (long) t ? String.format(java.util.Locale.ROOT, "%dt", (long) t) : String.format(java.util.Locale.ROOT, "%.2ft", t).replaceAll("\\.?0+t$", "t"));
      } else if (abs >= 1_000_000_000.0) {
         double b = abs / 1_000_000_000.0;
         return sign + (b == (long) b ? String.format(java.util.Locale.ROOT, "%db", (long) b) : String.format(java.util.Locale.ROOT, "%.2fb", b).replaceAll("\\.?0+b$", "b"));
      } else if (abs >= 1_000_000.0) {
         double m = abs / 1_000_000.0;
         return sign + (m == (long) m ? String.format(java.util.Locale.ROOT, "%dm", (long) m) : String.format(java.util.Locale.ROOT, "%.2fm", m).replaceAll("\\.?0+m$", "m"));
      } else if (abs >= 1_000.0 && abs == (long) abs && (long) abs % 1000 == 0) {
         double k = abs / 1_000.0;
         return sign + (k == (long) k ? String.format(java.util.Locale.ROOT, "%dk", (long) k) : String.format(java.util.Locale.ROOT, "%.2fk", k).replaceAll("\\.?0+k$", "k"));
      } else {
         if (val == (long) val) {
            return String.format(java.util.Locale.ROOT, "%d", (long) val);
         } else {
            return String.format(java.util.Locale.ROOT, "%.4f", val).replaceAll("\\.?0+$", "");
         }
      }
   }
