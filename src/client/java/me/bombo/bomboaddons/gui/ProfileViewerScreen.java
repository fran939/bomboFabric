...
public class ProfileViewerScreen extends Screen {
   ...
   private void drawMiningTab(GuiGraphicsExtractor graphics, Font font, int px, int py, int mouseX, int mouseY) {
      ...
      if (this.currentSubTab != null && "HOTM".equals(this.currentSubTab.id)) {
         ...
         for(HotMNodeInfo node : nodes) {
            ...
            if (level > 0) {
               if (node.isAbility) {
                  icon = new ItemStack(Items.DYED_SHULKER_BOX.pick(net.minecraft.world.item.DyeColor.PINK));
               } else if (level >= node.maxLevel) {
                  icon = new ItemStack(Items.EMERALD_BLOCK);
               } else {
                  icon = new ItemStack(Items.DYE.pick(net.minecraft.world.item.DyeColor.PINK));
               }
            } else {
         ...
         Item[] crystals = new Item[]{Items.DYE.pick(net.minecraft.world.item.DyeColor.LIME), Items.DYE.pick(net.minecraft.world.item.DyeColor.PURPLE), Items.DYE.pick(net.minecraft.world.item.DyeColor.YELLOW), Items.DYE.pick(net.minecraft.world.item.DyeColor.BLUE), Items.DYE.pick(net.minecraft.world.item.DyeColor.ORANGE), Items.DYE.pick(net.minecraft.world.item.DyeColor.RED), Items.DYE.pick(net.minecraft.world.item.DyeColor.MAGENTA), Items.QUARTZ, Items.DYE.pick(net.minecraft.world.item.DyeColor.CYAN), Items.DYE.pick(net.minecraft.world.item.DyeColor.BROWN), Items.DYE.pick(net.minecraft.world.item.DyeColor.GREEN), Items.DYE.pick(net.minecraft.world.item.DyeColor.BLACK)};
   ...
   private void drawGenericTab(GuiGraphicsExtractor graphics, Font font, int px, int py, int mouseX, int mouseY) {
      ...
      // Armor & Equipment: ONLY shown on MAIN inventory subcategory
      if (this.currentSubTab != null && "MAIN".equals(this.currentSubTab.id)) {
         for(int i = 0; i < 4; ++i) {
            int ay = aeY + i * 18;
      ...
      if (maxPages > 1 && !"BACKPACKS".equals(this.currentSubTab != null ? this.currentSubTab.id : "")) {
         int bw = 16;
      ...
      // If viewing Backpacks, render 2 columns of 9 backpacks on the left (Backpacks 1-18)
      if (this.currentSubTab != null && "BACKPACKS".equals(this.currentSubTab.id)) {
         int bpBaseX = px + 40;
         int bpBaseY = py + 25;
         Object[] bpKeys = this.data.backpacks.keySet().toArray();
         int totalBp = Math.max(18, bpKeys.length);
         for (int i = 0; i < 18; i++) {
            int col = i / 9; // 0 = Backpacks 1-9, 1 = Backpacks 10-18
            int row = i % 9;
            int bx = bpBaseX + col * 26;
            int by = bpBaseY + row * 23;
            boolean isSel = this.innerPage == i;
            boolean hasData = (this.data.backpacks.containsKey(i + 1) && this.data.backpacks.get(i + 1) != null && !this.data.backpacks.get(i + 1).isEmpty())
               || (this.data.backpacks.containsKey(i) && this.data.backpacks.get(i) != null && !this.data.backpacks.get(i).isEmpty());
            graphics.fill(bx, by, bx + 22, by + 21, isSel ? 0xFF9E6524 : (hasData ? 0xAA20242C : 0x4420242C));
            graphics.outline(bx, by, 22, 21, isSel ? 0xFFFFC266 : (hasData ? 0xFF556677 : 0xFF333333));
            ItemStack bpIcon = new ItemStack(Items.CHEST);
            graphics.item(bpIcon, bx + 3, by + 2);
            String numStr = String.valueOf(i + 1);
            graphics.text(font, (isSel ? "§e" : (hasData ? "§f" : "§8")) + numStr, bx + 22 - font.width(numStr) - 1, by + 11, -1, true);

            if (mouseX >= bx && mouseX <= bx + 22 && mouseY >= by && mouseY <= by + 21) {
               graphics.setTooltipForNextFrame(font, List.of(Component.literal("§6Backpack #" + (i + 1) + (hasData ? " §a(Loaded)" : " §8(Empty)"))), Optional.empty(), mouseX, mouseY);
            }
         }
      }
      ...
      int idx = 0;
      if (this.currentSubTab != null && "BACKPACKS".equals(this.currentSubTab.id)) {
         rows = Math.min(6, Math.max(1, (items.size() + 8) / 9));
      }
   ...
   private int selectedPetIdx = 0;

   private static final double[] PET_XP_TABLE = new double[]{
      0, 100, 210, 330, 460, 605, 765, 940, 1130, 1340, 1570, 1820, 2095, 2395, 2725, 3085, 3480, 3915, 4395, 4925,
      5510, 6155, 6865, 7645, 8495, 9425, 10440, 11545, 12745, 14045, 15450, 16965, 18595, 20345, 22220, 24225, 26365,
      28645, 31070, 33645, 36375, 39265, 42320, 45545, 48945, 52525, 56290, 60245, 64395, 68745, 73300, 78065, 83045,
      88245, 93670, 99325, 105215, 111345, 117720, 124345, 131225, 138365, 145770, 153445, 161395, 169625, 178140, 186945,
      196045, 205445, 215150, 225165, 235495, 246145, 257120, 268425, 280065, 292045, 304370, 317045, 330075, 343465, 357220,
      371345, 385845, 400725, 416000, 431675, 447755, 464245, 481150, 498475, 516225, 534405, 553020, 572075, 591575, 611525,
      631930, 652795, 674125
   };

   private LevelProgress getPetLevelAndProgress(double exp, String tier, String petType) {
      int maxLevel = "GOLDEN_DRAGON".equalsIgnoreCase(petType) ? 200 : 100;
      double mult = "LEGENDARY".equalsIgnoreCase(tier) || "MYTHIC".equalsIgnoreCase(tier) ? 1.0 :
                    "EPIC".equalsIgnoreCase(tier) ? 0.8 :
                    "RARE".equalsIgnoreCase(tier) ? 0.6 :
                    "UNCOMMON".equalsIgnoreCase(tier) ? 0.4 : 0.2;
      double currentExp = exp;
      for (int lvl = 1; lvl <= maxLevel; lvl++) {
         int idx = Math.min(lvl, PET_XP_TABLE.length - 1);
         double needed = PET_XP_TABLE[idx] * mult;
         if (currentExp < needed || lvl == maxLevel) {
            float prog = (lvl == maxLevel) ? 1.0f : (float)(currentExp / Math.max(1.0, needed));
            return new LevelProgress(lvl, Math.min(1.0f, Math.max(0.0f, prog)));
         }
         currentExp -= needed;
      }
      return new LevelProgress(maxLevel, 1.0f);
   }

   private ItemStack getPetItemStack(ProfileFetcher.Pet pet) {
      if (pet == null) return new ItemStack(Items.BONE);
      String petId = "PET_" + pet.type.toUpperCase() + "_" + pet.tier.toUpperCase();
      SkyblockItemManager.SkyblockItemInfo info = SkyblockItemManager.getInfo(petId);
      if (info == null) {
         info = SkyblockItemManager.getInfo(pet.type.toUpperCase() + ";" + (pet.tier.equals("LEGENDARY") ? "4" : "3"));
      }
      if (info != null) {
         return SkyblockItemManager.createSkyblockItem(info.id);
      }
      if (pet.type.contains("DRAGON")) return new ItemStack(Items.DRAGON_HEAD);
      if (pet.type.contains("ZOMBIE")) return new ItemStack(Items.ZOMBIE_HEAD);
      if (pet.type.contains("CREEPER")) return new ItemStack(Items.CREEPER_HEAD);
      if (pet.type.contains("SKELETON")) return new ItemStack(Items.SKELETON_SKULL);
   ...
   private void drawPetsTab(GuiGraphicsExtractor graphics, Font font, int px, int py, int mouseX, int mouseY) {
      ...
      // Draw Pet Grid (Left side)
      for (int i = 0; i < totalSlots; i++) {
         int petIdx = this.scrollOffset + i;
         int c = i % cols;
         int r = i / cols;
         int slotX = gridX + c * 26;
         int slotY = gridY + r * 26;

         boolean hasPet = petIdx < this.data.pets.size();
         boolean isSelected = hasPet && (this.selectedPetIdx == petIdx);

         graphics.fill(slotX, slotY, slotX + 24, slotY + 24, isSelected ? 0x8838BDF8 : 0x55181C24);
         graphics.outline(slotX, slotY, 24, 24, isSelected ? 0xFF38BDF8 : 0xFF2D3748);

         if (hasPet) {
            ProfileFetcher.Pet pet = this.data.pets.get(petIdx);
            ItemStack petStack = getPetItemStack(pet);
            graphics.item(petStack, slotX + 4, slotY + 4);

            LevelProgress lp = getPetLevelAndProgress(pet.exp, pet.tier, pet.type);
            String lvlStr = String.valueOf(lp.level);
            graphics.fill(slotX + 13 - font.width(lvlStr) / 2, slotY + 14, slotX + 23, slotY + 23, 0xCC000000);
            graphics.text(font, "§f" + lvlStr, slotX + 23 - font.width(lvlStr), slotY + 15, -1, true);

            if (pet.active) {
               graphics.fill(slotX + 1, slotY + 1, slotX + 5, slotY + 5, 0xFF22C55E); // Green dot for active
            }

            if (mouseX >= slotX && mouseX <= slotX + 24 && mouseY >= slotY && mouseY <= slotY + 24) {
               this.selectedPetIdx = petIdx;
               List<Component> tooltip = new ArrayList<>();
               String tierColor = pet.tier.equals("LEGENDARY") ? "§6" : (pet.tier.equals("MYTHIC") ? "§d" : (pet.tier.equals("EPIC") ? "§5" : (pet.tier.equals("RARE") ? "§9" : (pet.tier.equals("UNCOMMON") ? "§a" : "§f"))));
               tooltip.add(Component.literal(tierColor + "[Lvl " + lp.level + "] " + pet.type.replace("_", " ")));
               tooltip.add(Component.literal("§7Total XP: §e" + String.format("%,.0f", pet.exp)));
               if (pet.heldItem != null && !pet.heldItem.isEmpty()) {
                  tooltip.add(Component.literal("§7Held Item: §b" + pet.heldItem.replace("_", " ")));
               }
               if (pet.active) {
                  tooltip.add(Component.literal("§a✔ Currently Active"));
               }
               graphics.setTooltipForNextFrame(font, tooltip, Optional.empty(), mouseX, mouseY);
            }
         }
      ...
      // Draw Info Card on Right Side
      int cardX = gridX + cols * 26 + 15;
      int cardY = gridY;
      int cardW = 210;
      int cardH = 180;

      graphics.fill(cardX, cardY, cardX + cardW, cardY + cardH, 0x88181C24);
      graphics.outline(cardX, cardY, cardW, cardH, 0xFF3C4452);
      graphics.fill(cardX + 1, cardY + 1, cardX + cardW - 1, cardY + 20, 0xAA202632);
      graphics.text(font, "§6§lPet Details", cardX + 8, cardY + 6, -1, true);

      if (this.selectedPetIdx >= 0 && this.selectedPetIdx < this.data.pets.size()) {
         ProfileFetcher.Pet pet = this.data.pets.get(this.selectedPetIdx);
         LevelProgress lp = getPetLevelAndProgress(pet.exp, pet.tier, pet.type);
         String tierColor = pet.tier.equals("LEGENDARY") ? "§6" : (pet.tier.equals("MYTHIC") ? "§d" : (pet.tier.equals("EPIC") ? "§5" : (pet.tier.equals("RARE") ? "§9" : (pet.tier.equals("UNCOMMON") ? "§a" : "§f"))));

         // Pet Head & Name
         ItemStack petStack = getPetItemStack(pet);
         graphics.item(petStack, cardX + 10, cardY + 28);
         graphics.text(font, tierColor + "§l" + pet.type.replace("_", " "), cardX + 34, cardY + 30, -1, true);
         graphics.text(font, "§7Rarity: " + tierColor + pet.tier, cardX + 34, cardY + 42, -1, true);

         // Level & Progress Bar
         int py2 = cardY + 62;
         graphics.text(font, "§eLevel " + lp.level + (lp.level >= (pet.type.equalsIgnoreCase("GOLDEN_DRAGON") ? 200 : 100) ? " §6(MAX)" : ""), cardX + 10, py2, -1, true);
         py2 += 12;
         drawSegmentedProgressBar(graphics, cardX + 10, py2, cardW - 20, 6, lp.progress, lp.level >= 100 ? 0xFFFFB703 : 0xFF38BDF8);
         py2 += 14;
         graphics.text(font, "§7Total XP: §f" + String.format("%,.0f", pet.exp), cardX + 10, py2, -1, true);

         // Held Item
         py2 += 22;
         graphics.fill(cardX + 10, py2 - 4, cardX + cardW - 10, py2 + 26, 0x442D3748);
         graphics.outline(cardX + 10, py2 - 4, cardW - 20, 30, 0xFF3C4452);
         if (pet.heldItem != null && !pet.heldItem.isEmpty()) {
            ItemStack heldStack = SkyblockItemManager.createSkyblockItem(pet.heldItem);
            if (heldStack.isEmpty()) heldStack = new ItemStack(Items.NAME_TAG);
            graphics.item(heldStack, cardX + 14, py2 + 2);
            graphics.text(font, "§b" + pet.heldItem.replace("_", " "), cardX + 36, py2 + 6, -1, true);
         } else {
            graphics.text(font, "§8No Held Item", cardX + 16, py2 + 6, -1, true);
         }

         // Status
         py2 += 36;
         if (pet.active) {
            graphics.text(font, "§a● Currently Summoned", cardX + 10, py2, -1, true);
         } else {
            graphics.text(font, "§7○ In Pet Menu", cardX + 10, py2, -1, true);
         }
      } else {
         graphics.text(font, "§7Select a pet to view info", cardX + 15, cardY + 50, -1, true);
      }
   }
   ...
   private List<ItemStack> getActiveItems() {
      if (this.currentSubTab == null) {
         ...
         if ("BACKPACKS".equals(this.currentSubTab.id)) {
            int bpNum = this.innerPage + 1;
            if (this.data.backpacks.containsKey(bpNum)) {
               return this.data.backpacks.get(bpNum);
            }
            if (this.data.backpacks.containsKey(this.innerPage)) {
               return this.data.backpacks.get(this.innerPage);
            }
   ...
   public boolean mouseClicked(MouseButtonEvent event, boolean handled) {
      ...
      if (this.currentTab == ProfileViewerScreen.Tab.PETS) {
         ...
         for (int i = 0; i < cols * rows; i++) {
            int petIdx = this.scrollOffset + i;
            if (petIdx >= this.data.pets.size()) break;
            int c = i % cols;
            int r = i / cols;
            int slotX = gridX + c * 26;
            int slotY = gridY + r * 26;
            if (event.x() >= slotX && event.x() <= slotX + 24 && event.y() >= slotY && event.y() <= slotY + 24) {
               this.selectedPetIdx = petIdx;
               return true;
            }
         }
      ...
      if (tabs != null) {
         ...
         if (this.currentSubTab != null && "BACKPACKS".equals(this.currentSubTab.id)) {
            ...
            for (int i = 0; i < 18; i++) {
               ...
               int bx = bpBaseX + col * 26;
               int by = bpBaseY + row * 23;
               if (event.x() >= (double)bx && event.x() <= (double)(bx + 22) && event.y() >= (double)by && event.y() <= (double)(by + 21)) {
                  this.innerPage = i;
                  return true;
               }
            }
   ...
   private void drawRiftTab(GuiGraphicsExtractor graphics, Font font, int px, int py, int mouseX, int mouseY) {
      ...
      for(int i = 0; i < charmNames.length; ++i) {
         ...
         int drawX = cx + i % 4 * 20;
         int drawY = cy + i / 4 * 20;
         ItemStack icon = found != null ? new ItemStack(charmItems[i]) : new ItemStack(Items.DYE.pick(net.minecraft.world.item.DyeColor.GRAY));
