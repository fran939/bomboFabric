...
public class InventoryManager {
    ...
    public static void openSnapshot(String name, int index) {
        ...
        if (matches.isEmpty()) {
            ...
            if (!snapshots.isEmpty()) {
                ...
                for (int i = 0; i < Math.min(snapshots.size(), 5); i++) {
                    mc.player.sendSystemMessage(Component.literal("§7- " + snapshots.get(i).guiName));
                }
                if (snapshots.size() > 5) {
                    mc.player.sendSystemMessage(Component.literal("§7... and " + (snapshots.size() - 5) + " more."));
                }
            }
            return;
        }
        ...
        if (index > matches.size()) {
            mc.player.sendSystemMessage(Component.literal("§c[BomboAddons] Only " + matches.size()
                    + " matches found for '" + name + "', but index " + index + " was requested."));
            return;
        }
