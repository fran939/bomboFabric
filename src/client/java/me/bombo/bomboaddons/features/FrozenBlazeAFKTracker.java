...
public class FrozenBlazeAFKTracker {
   ...
   private static float anchorYaw = 0;
   private static float anchorPitch = 0;
   ...
   public static void onTick(Minecraft mc) {
      ...
      // Check if player moved horizontally AND moved camera rotation
      double dx = px - anchorX;
      double dz = pz - anchorZ;
      double distSq = dx * dx + dz * dz;

      float dYaw = Math.abs(yaw - anchorYaw);
      if (dYaw > 180.0F) dYaw = 360.0F - dYaw;
      float dPitch = Math.abs(pitch - anchorPitch);
      boolean cameraMoved = (dYaw >= 5.0F || dPitch >= 5.0F);
      boolean playerMoved = (distSq >= 0.25);

      // Only mark as not AFK if player moved BOTH position and camera
   ...
   public static void drawTimerInfo(GuiGraphicsExtractor g, int x, int y, float scale, boolean preview) {
      ...
      if (preview) {
         ...
         long now = System.currentTimeMillis();
         long elapsedMs = Math.max(0L, now - lastMovedTime);
   ...
   public static boolean isWearingFrozenBlaze(Minecraft mc) {
      ...
      if (headChanged || chestChanged || legsChanged || feetChanged || !cachedWearingFB) {
   ...
   private static boolean isFrozenBlazePiece(ItemStack stack, EquipmentSlot slot) {
      ...
      // 1. Check Skyblock ID
      String sbId = SkyblockUtils.getSkyblockId(stack);
      if (sbId != null && sbId.toUpperCase().contains("FROZEN_BLAZE")) {
         return true;
      }

      // 2. Check CustomData / ExtraAttributes
   ...
   private static void triggerWarning(Minecraft mc, BomboConfig.Settings s) {
      ...
      if (s.fbWarnTitle && mc.gui != null) {
         mc.gui.hud.setTitle(Component.literal("§c§lMOVE! §e(Frozen Blaze AFK)"));
         mc.gui.hud.setSubtitle(Component.literal("§7Stationary for §e" + s.fbWarnSeconds + "s§7!"));
      }
      if (s.fbWarnChat) {
