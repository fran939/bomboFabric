...
public abstract class SearchableGridWidget extends AbstractContainerWidget {
	...
	public void setWidth(int width) {
		if (packed) {
			int perRow = Math.min(Math.max(1, (width - AbstractScrollArea.SCROLLBAR_WIDTH) / expectedWidgetWidth), maxPerRow);
	...
	private class WidgetsContainer extends AbstractContainerWidget {
		...
		protected void recreateGrid() {
			...
			int columns = Math.max(1, (getWidth() - AbstractScrollArea.SCROLLBAR_WIDTH) / expectedWidgetWidth);
