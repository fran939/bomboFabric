...
public class DojoUtilities {
      ...
      for (int x = -radiusXZ; x <= radiusXZ; x++) {
         for (int z = -radiusXZ; z <= radiusXZ; z++) {
            for (int y = -1; y <= 4; y++) {
               ...
               int stage = 0;
               if (state.is(net.minecraft.world.level.block.Blocks.WOOL.pick(net.minecraft.world.item.DyeColor.GREEN))) {
                  stage = 1;
               } else if (state.is(net.minecraft.world.level.block.Blocks.WOOL.pick(net.minecraft.world.item.DyeColor.YELLOW))) {
                  stage = 2;
               } else if (state.is(net.minecraft.world.level.block.Blocks.WOOL.pick(net.minecraft.world.item.DyeColor.RED))) {
                  stage = 3;
               }
   ...
   public static void renderTracers(net.fabricmc.fabric.api.client.rendering.v1.level.LevelRenderContext context) {
      ...
      net.minecraft.world.phys.Vec3 camPos = mc.gameRenderer.mainCamera().position();
      com.mojang.blaze3d.vertex.PoseStack poseStack = context.poseStack();
      me.bombo.bomboaddons.OrderedSubmitNodeCollector collector = new me.bombo.bomboaddons.OrderedSubmitNodeCollector(context.submitNodeCollector());
      org.joml.Vector3fc look = mc.gameRenderer.mainCamera().forwardVector();
      float startX = look.x();
      float startY = look.y();
      float startZ = look.z();
