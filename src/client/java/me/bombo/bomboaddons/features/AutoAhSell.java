...
public class AutoAhSell {
   ...
   public static void start(String priceStr) {
      ...
      if (parsed <= 0L) {
         mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §cInvalid price: §e" + priceStr + "§c. Use formats like §e4m§c, §e500k§c, §e1.5m§c, or numbers."));
         return;
      }
      ...
      if (targetStack == null || targetStack.isEmpty()) {
         mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §cHover over the item you want to sell in your inventory or hold it in your hand!"));
         return;
      }
      ...
      mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §aStarting Auto Sell for §f" + targetDisplayName + " §7(ID: §e" + targetSkyblockId + "§7) at §6" + targetPriceFormatted + " coins§a..."));
   }
   ...
   public static void onConfirmWarning() {
      ...
      Minecraft mc = Minecraft.getInstance();
      if (mc.player != null) {
         mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §aPrice warning bypassed for this session. Continuing sale..."));
      }
   ...
   public static void stop() {
      ...
      Minecraft mc = Minecraft.getInstance();
      if (mc.player != null) {
         mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §cAuto Sell cancelled."));
      }
   }
      ...
      switch (currentStep) {
         ...
         case WAIT_AH_OPEN -> {
            ...
            if (++actionAttempts > 40) { // 40 * delay timeout
               mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §cTimed out waiting for Auction House to open."));
               stop();
            }
         }
         ...
         case CLICK_ITEM_SLOT -> {
            if (screen instanceof AbstractContainerScreen<?> containerScreen) {
               ...
               if (matchingPlayerSlot == -1) {
                  mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §aNo more matching §f" + targetDisplayName + " §afound in inventory. Finished!"));
                  running = false;
                  currentStep = Step.IDLE;
                  return;
               }
            ...
            if (screen instanceof AbstractContainerScreen<?> containerScreen) {
               ...
               if (!sessionWarnBypassed && marketPrice > 0L && targetPrice < (marketPrice * 40L / 100L)) {
                  if (!awaitingConfirmBypass) {
                     ...
                     mc.gui.hud.setTitle(Component.literal("§c§lPRICE TOO LOW!"));
                     mc.gui.hud.setSubtitle(Component.literal("§eSelling for " + targetPriceFormatted + " §7(Market: §a" + LowestBinManager.formatPrice(marketPrice) + "§7)"));
                     mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §c§lWARNING: §eYour price of §6" + targetPriceFormatted + " §eis significantly lower than the estimated market price (§a" + LowestBinManager.formatPrice(marketPrice) + "§e)!"));
                     mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §7Type §b/b sell bypass §7or manually click Confirm to proceed anyway and disable this warning for this session."));
                  }
                  return;
               }
         ...
         case CHECK_NEXT_ITEM -> {
            if (screen instanceof AbstractContainerScreen<?> containerScreen) {
               ...
               if (remaining != -1) {
                  ...
                  mc.player.sendSystemMessage(Component.literal("§8[§3Bombo§8]§r §aAll matching items sold successfully!"));
                  running = false;
                  currentStep = Step.IDLE;
               }
            } else {
