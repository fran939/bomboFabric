...
public class SBECommands {
   ...
   public static void handleCommand(String command, String name, String profile) {
      ...
      HttpRequest.Builder reqBuilder = HttpRequest.newBuilder()
            .uri(URI.create(url))
            .header("User-Agent", "Mozilla/5.0 (Bomboaddons 1.21.11) " + selfUuid)
   ...
   private static void parseAndDisplay(String body, String command, String name) {
      try {
         ...
         if (json.has("motive") && json.get("motive").isJsonPrimitive()) {
            String motive = json.get("motive").getAsString();
            String err = json.has("error") && json.get("error").isJsonPrimitive() ? json.get("error").getAsString() : "API Key Suspended";
            sendMessage("§c[BomboAPI] " + err + "! Motive: §e" + motive);
            return;
         }
         if (json.has("error") && json.get("error").isJsonPrimitive()) {
            ...
            if (json.has("message") && json.get("message").isJsonPrimitive()) {
               sendMessage("§c[BomboAPI] " + err + ": §e" + json.get("message").getAsString());
            } else {
               sendMessage("§c[BomboAPI] " + err);
            }
            return;
         }
