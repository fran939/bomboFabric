...
public class FeastBakeryHud {
   ...
   public static void onHudRender(GuiGraphicsExtractor g) {
      ...
      if (currentFrame != lastRenderFrame || mc.gui.screen() instanceof HudMoveScreen) {
         lastRenderFrame = currentFrame;
         BomboConfig.Settings s = BomboConfig.get();
         if (s.feastBakeryHud) {
            LowestBinManager.ensureLoaded();
            boolean inMenu = false;
            Screen var7 = mc.gui.screen();
   ...
   public static void drawBakeryInfo(GuiGraphicsExtractor g, int x, int y, List<DetectedItem> items) {
      ...
      BomboConfig.HudStyle style = BomboConfig.get().getHudStyle("BAKERY");
      int bgColor = style.getBackgroundColorInt();
      int borderColor = style.getBorderColorInt();
      g.fill(-5, -5, width + 5, height + 5, bgColor);
      g.outline(-5, -5, width + 10, height + 10, borderColor);

      g.text(font, "§lFeast Bakery §r§7- §bScott", 0, 0, style.getTitleColorInt(), true);
      g.fill(0, 11, width, 12, borderColor);
      int curY = 18;
