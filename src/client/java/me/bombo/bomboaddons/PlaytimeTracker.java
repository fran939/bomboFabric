...
public class PlaytimeTracker {
         ...
         try {
            ...
            try {
               ...
               if (loaded != null) {
                  ...
                  loaded.forEach((k, v) -> {
                     if (v != null) {
                        v.dailyTime.replaceAll((d, t) -> Math.min(86400000L, Math.max(0L, t)));
                        v.dailyAfk.replaceAll((d, t) -> Math.min(86400000L, Math.max(0L, t)));
                     }
   ...
   private static void mergeData(AreaData target, AreaData source) {
      ...
      target.totalTime = Math.max(target.totalTime, source.totalTime);
      target.afkTime = Math.max(target.afkTime, source.afkTime);
      source.dailyTime.forEach((date, time) -> {
         long existing = target.dailyTime.getOrDefault(date, 0L);
         long val = Math.min(86400000L, Math.max(existing, time));
         target.dailyTime.put(date, val);
      });
      source.dailyAfk.forEach((date, time) -> {
         long existing = target.dailyAfk.getOrDefault(date, 0L);
         long val = Math.min(86400000L, Math.max(existing, time));
         target.dailyAfk.put(date, val);
      });
