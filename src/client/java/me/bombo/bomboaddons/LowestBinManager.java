...
public class LowestBinManager {
   ...
   private static long getRawPrice(String id) {
      ...
      if (bazaarCache.containsKey(id) || bazaarSellCache.containsKey(id)) {
         double price = 0.0;
         if (preferSell) {
            price = bazaarSellCache.getOrDefault(id, 0.0);
            if (price <= 0.0) price = bazaarCache.getOrDefault(id, 0.0);
         } else {
            price = bazaarCache.getOrDefault(id, 0.0);
            if (price <= 0.0) price = bazaarSellCache.getOrDefault(id, 0.0);
         }
         if (price > 0.0) {
      ...
      if (id.startsWith("SHARD_") || id.endsWith("_SHARD")) {
         ...
         if (bazaarCache.containsKey(alt) || bazaarSellCache.containsKey(alt)) {
            double price = 0.0;
            if (preferSell) {
               price = bazaarSellCache.getOrDefault(alt, 0.0);
               if (price <= 0.0) price = bazaarCache.getOrDefault(alt, 0.0);
            } else {
               price = bazaarCache.getOrDefault(alt, 0.0);
               if (price <= 0.0) price = bazaarSellCache.getOrDefault(alt, 0.0);
            }
            if (price > 0.0) {
               return Math.round(price);
            }
      ...
      if (priceCache.containsKey(id)) {
         ...
         if (id.contains(";")) {
            ...
            if (bazaarCache.containsKey(baseId) || bazaarSellCache.containsKey(baseId)) {
               double price = 0.0;
               if (preferSell) {
                  price = bazaarSellCache.getOrDefault(baseId, 0.0);
                  if (price <= 0.0) price = bazaarCache.getOrDefault(baseId, 0.0);
               } else {
                  price = bazaarCache.getOrDefault(baseId, 0.0);
                  if (price <= 0.0) price = bazaarSellCache.getOrDefault(baseId, 0.0);
               }
               if (price > 0.0) {
                  return Math.round(price);
               }
   ...
   public static class ValueEntry {
      ...
      public ValueEntry(String label, long price, String source) {
         this.label = label;
         this.price = price;
         this.source = source;
      }
   ...
   private static final Map<String, String> REFORGE_STONES = Map.ofEntries(
      ...
      Map.entry("hyper", "END_STONE_GEODE"),
      Map.entry("undead", "PREMIUM_FLESH"),
   ...
   public static ItemValueBreakdown calculateDetailedEstimatedValue(net.minecraft.world.item.ItemStack stack, String skyblockId) {
      ItemValueBreakdown breakdown = new ItemValueBreakdown();
      if (stack == null || stack.isEmpty()) return breakdown;
      ...
      if (preferCheapest && craftCost > 0L && (ahPrice <= 0L || craftCost < ahPrice)) {
         basePrice = craftCost;
         baseSource = "Craft Cost";
      } else if (ahPrice > 0L) {
         basePrice = ahPrice;
         baseSource = isBazaar(skyblockId) ? "Bazaar" : "Lowest BIN";
      } else if (craftCost > 0L) {
         basePrice = craftCost;
         baseSource = "Craft Cost";
      }

      if (basePrice > 0L) {
         breakdown.baseItem = new ValueEntry("Base Item", basePrice, baseSource);
         breakdown.totalPrice += basePrice;
      }

      // 2. ExtraAttributes Upgrades & Enchants
      try {
         net.minecraft.world.item.component.CustomData customData = (net.minecraft.world.item.component.CustomData)stack.get(net.minecraft.core.component.DataComponents.CUSTOM_DATA);
         if (customData != null) {
            ...
            if (extra != null) {
               ...
               int hpbCount = extra.getInt("hot_potato_count").orElse(0);
               if (hpbCount > 0) {
   ...
   public static long calculateEstimatedValue(net.minecraft.world.item.ItemStack stack, String skyblockId) {
      ItemValueBreakdown breakdown = calculateDetailedEstimatedValue(stack, skyblockId);
