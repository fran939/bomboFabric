...
public class DwarvenCarpetReplacer {
         ...
         for (int z = -radius; z <= radius; z++) {
            for (int y = -3; y <= 3; y++) {
               ...
               if (mc.level.isLoaded(mpos)) {
                  ...
                  if (state.is(Blocks.CARPET.pick(net.minecraft.world.item.DyeColor.GRAY)) || state.is(Blocks.CARPET.pick(net.minecraft.world.item.DyeColor.LIGHT_GRAY))) {
                     mc.level.setBlock(mpos.immutable(), Blocks.CARPET.pick(net.minecraft.world.item.DyeColor.RED).defaultBlockState(), 19);
                  }
               }
            }
         }
   ...
   public static BlockState checkAndReplace(BlockState state) {
      ...
      if (!state.is(Blocks.CARPET.pick(net.minecraft.world.item.DyeColor.GRAY)) && !state.is(Blocks.CARPET.pick(net.minecraft.world.item.DyeColor.LIGHT_GRAY))) return state;
      String area = SkyblockUtils.getLocation();
      if (area == null) area = BomboaddonsClient.currentArea;
      if (area != null && (area.toLowerCase().contains("dwarven") || area.toLowerCase().contains("mines"))) {
         return Blocks.CARPET.pick(net.minecraft.world.item.DyeColor.RED).defaultBlockState();
      }
      return state;
   }
}
