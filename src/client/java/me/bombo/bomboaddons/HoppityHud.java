...
public class HoppityHud {
   ...
   public static int getHudWidth() {
      ...
      Font font = mc.font;
      int maxW = font != null ? font.width("§6§lHoppity Eggs") : 90;
      for (EggFinder.EggType type : EggFinder.EggType.values()) {
         if (font != null) {
            maxW = Math.max(maxW, font.width(type.name + " Egg"));
         }
      }
      return maxW + 4;
   }
   ...
   public static int getHudHeight() {
      int count = EggFinder.EggType.values().length;
      return 12 + count * 11;
   }
   ...
   public static void drawHud(GuiGraphicsExtractor g, int baseX, int baseY, boolean isDummy) {
      Minecraft mc = Minecraft.getInstance();
      Font font = mc.font;
      if (font == null) return;
      ...
      List<EggFinder.EggWaypoint> waypoints = isDummy ? new ArrayList<>() : EggFinder.getActiveWaypoints();
      int curY = baseY + 12;
