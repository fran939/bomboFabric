...
public class CustomBindsProcessor {
   ...
   public static java.util.List<BomboConfig.CommandBind> getInGameBinds() {
      ...
      if (s.keybindBinds != null) {
         List<BomboConfig.CommandBind> gen = s.keybindBinds.get("General");
         if (gen != null) list.addAll(gen);
         if (!"General".equalsIgnoreCase(activeProf)) {
   ...
   public static java.util.List<BomboConfig.CommandBind> getGuiProfileBinds() {
      ...
      if (s.profileBinds != null) {
         List<BomboConfig.CommandBind> gen = s.profileBinds.get("General");
         if (gen != null) list.addAll(gen);
         if (!"General".equalsIgnoreCase(activeProf)) {
   ...
   public static boolean checkGuiKeybinds(int keyCode) {
      ...
      if (now - lastGuiKeybindHandledTime < 250L && lastGuiKeybindHandledKey == keyCode) {
         ...
         if (mc.gui.screen() == null) {
            return false;
         } else if (!(mc.gui.screen() instanceof ChatScreen) && !(mc.gui.screen() instanceof AbstractSignEditScreen) && !(mc.gui.screen() instanceof me.bombo.bomboaddons.gui.config.BomboConfigScreen) && !(mc.gui.screen() instanceof BomboConfigGUI) && !(mc.gui.screen() instanceof me.bombo.bomboaddons.HudMoveScreen)) {
   ...
   public static boolean isSingleKeyCurrentlyPressed(String singleKeyStr) {
      ...
      if (clean.startsWith("mouse") || clean.startsWith("button") || clean.equals("left_click") || clean.equals("right_click") || clean.equals("middle_click")) {
         ...
         if (mouseCode >= 1000 && mouseCode <= 1007) {
            ...
            return GLFW.glfwGetMouseButton(windowHandle, button) == 1 || pressedKeys.contains(mouseCode);
         }
      }
      ...
      if (code != -1) {
         if (code >= 1000 && code <= 1007) {
            return GLFW.glfwGetMouseButton(windowHandle, code - 1000) == 1 || pressedKeys.contains(code);
         }
   ...
   private static void showTitle(String titleText) {
      ...
      mc.execute(() -> {
         if (mc.gui != null) {
            mc.gui.hud.setTitle(Component.literal(titleText.replace("&", "Â§")));
         }
