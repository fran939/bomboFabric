...
public class TargetPests {
   ...
   public static String getPestName(Entity entity) {
      ...
      if (cached != null) {
         ...
         String pestName = getPestNameInternal(entity);
   ...
   private static String getHeadTextureValueInternal(Entity entity) {
      if (entity instanceof net.minecraft.world.entity.Display.ItemDisplay itemDisplay) {
         String tex = getHeadTextureFromStack(itemDisplay.getItemStack());
   ...
   public static String getDirectHeadTextureValue(Entity entity) {
      if (entity == null) return null;
      if (entity instanceof net.minecraft.world.entity.Display.ItemDisplay itemDisplay) {
         return getHeadTextureFromStack(itemDisplay.getItemStack());
      }
   ...
   private static String getPestNameInternal(Entity entity) {
      String headTex = getHeadTextureValue(entity);
      if (headTex == null && entity instanceof LivingEntity living) {
         ItemStack head = living.getItemBySlot(EquipmentSlot.HEAD);
         headTex = getHeadTextureFromStack(head);
      }
      if (headTex == null && entity instanceof net.minecraft.world.entity.Display.ItemDisplay itemDisplay) {
         headTex = getHeadTextureFromStack(itemDisplay.getItemStack());
      }
      if (headTex != null) {
         BomboConfig.Settings s = BomboConfig.get();
         if ((s.debugEntities || s.pestDebug || s.pestEspDebug) && SkyblockUtils.isInGarden()) {
            System.out.println("DEBUG: Pest Entity Texture: " + headTex);
         }
         if (PEST_MAP.containsKey(headTex)) {
            String pName = (String)PEST_MAP.get(headTex);
            if (s.pestDebug && SkyblockUtils.isInGarden()) {
               System.out.println("DEBUG: Matched pest by texture: " + pName);
            }
            return pName;
         }
         String hash = extractTextureHash(headTex);
         if (hash != null && PEST_MAP.containsKey(hash)) {
            String pName = (String)PEST_MAP.get(hash);
            if (s.pestDebug && SkyblockUtils.isInGarden()) {
               System.out.println("DEBUG: Matched pest by hash: " + pName + " (hash: " + hash + ")");
            }
            return pName;
         }
      }
   ...
   static {
      ...
      for (Map.Entry<String, String> entry : PEST_MAP.entrySet()) {
         String h = extractTextureHash(entry.getKey());
         if (h != null) hashes.put(h, entry.getValue());
      }
      PEST_MAP.putAll(hashes);
   }
